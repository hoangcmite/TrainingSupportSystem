                    ...
                    <form action = "ClassList" method="POST">
                        <div class="advanced-search">
                            <div class = "advanced-search-item secondd" id="search-1">
                                <label for="type" class="mr-sm-2">Filter by Term</label>
                                <select style="width: 200px" class="form-control" id="type" name = "term">
                                    <option value="" disabled selected hidden>Term</option>
                                    <c:forEach items="${lterm}" var="t">
                                        <option value = "${t.termId}">${t.getTermName()}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class = "advanced-search-item secondd" id="search-1">
                                <label for="type" class="mr-sm-2">Filter by Trainer</label>
                                <select style="width: 200px" class="form-control" id="type" name = "trainer">
                                    <option value="" disabled selected hidden>Trainer</option>
                                    <c:forEach items="${trainer}" var="t">
                                        <option value = "${t.userId}">${t.getFullname()}</option>
                                    </c:forEach>
                                </select>
                            ...
                            <div class = "advanced-search-item secondd" id="search-2">
                                ...
                                <select style="width: 200px" class="form-control" id="status" name = "status">
                                    ...
                                    <option ${requestScope.status == ""?"selected = selected":""} value = "2">Closed</option>
                                </select>
                            </div>

                            <div class = "advanced-search-item secondd" id="search-3">
                                <label for="title" class="mr-sm-2">Search by Code</label>
                                <input style="width: 230px" type="text" class="form-control" name ="txt"  id="title" placeholder="Enter Class code" >
                            </div>
                            ...
                            <div class = "btn-add" >
                                <button style="margin-left: -560px;" onclick="location.href = '/WebApplication2/AddClass';" class="btn btn-secondary" type="button">Add</button>
                            </div>
                        </div>
                    </form>
                    <!-- DataTables Example -->
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    <thead>
                                        <tr class = "bg-warning">
                                            ...
                                            <th style="text-align: center" scope="col">
                                                <span  onclick ="sort(${requestScope.sortType})"> Class code </span>
                                            </th>
                                            <th style="text-align: center" scope="col">
                                                <span onclick ="sort(${requestScope.sortTitle})"> Term </span>
                                            </th>
                                            <th style="text-align: center" scope="col">
                                                <span onclick ="sort(${requestScope.sortValue})"> Trainer </span>
                                            </th>
                                            <th style="text-align: center" scope="col">
                                                Status
                                    ...
                                    <tbody>
                                        <c:forEach items="${classlist}" var="c">
                                            <tr>
                                                ...
                                                <td>${c.getTermName()}</td>
                                                <td style="text-align: center">${c.getNameTrainer()}</td>   
                                                ...
                                                <c:if test="${c.getStatus() eq 2}">
                                                    <td style="text-align: center" id="change">
                                                        <button type="button" style="width: 80px; border-radius: 15px; background-color: gray; border-color: white; color: white; height: 35px;">Close</button>
                                                    </td>
                                                </c:if>
                                                <td style="text-align: center">
                                                    <button type="button" onclick="location.href = '';" class="csw-btn-button" >Edit</button>
                                                </td>     
                                ...
                                <div class = "setting-pagination">
                                    <ul class="pagination" id = "paggerBottom">
                                        <c:forEach begin="1" end="${endP}" var="i">
                                            <a class="page-link" href="ClassList?index=${i}">${i}</a>
                                        </c:forEach>
                                    </ul>
                                </div>
                            </div>
