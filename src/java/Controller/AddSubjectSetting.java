...
public class AddSubjectSetting extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddSubjectSetting</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddSubjectSetting at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<SubjectSetting> ltype = subjects.getTypeList();
        ...
        request.getRequestDispatcher("/view/manager/CreateNewSubjectSetting.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int subjectid = Integer.parseInt(request.getParameter("scode"));
        String des = request.getParameter("description");
        
        SubjectSetting s = new SubjectSetting();
        s.setType_id(type);
        s.setSettingTitle(title);
        s.setSettingValue(value);
        s.setDisplayOrder(order);
        s.setStatus(status);
        s.setSubjectId(subjectid);
        s.setDescription(des);
        subjects.insertSubjectSetting(s);
